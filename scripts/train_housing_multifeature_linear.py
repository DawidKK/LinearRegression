...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train multifeature linear regression on raw housing data."
    )
    parser.add_argument(
        "--target",
        default="price_usd",
    ...
    parser.add_argument(
        "--output-path",
        default="reports/figures/housing_multifeature_linear_regression.png",
...
def plot_results(
    ...
    # Right chart: coefficient sizes so you can interpret feature influence.
    coefficient_frame = pd.DataFrame(
        {"feature": feature_names, "coefficient": coefficients}
    ).sort_values("coefficient")

    axes[1].barh(coefficient_frame["feature"], coefficient_frame["coefficient"], color="#2ca02c")
    axes[1].set_title("Learned Coefficients (All Features)")
...
def main() -> None:
    ...
    print("Multifeature Housing Linear Regression")
    print("-" * 42)
